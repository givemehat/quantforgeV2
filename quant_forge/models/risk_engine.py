...
class GBM:
    ...
    def __init__(
        self, mu: float, sigma: float, S0: float, dt: float = 1 / 252, T: float = 1.0
    ):
        self.mu = mu
        self.sigma = sigma
        self.S0 = S0
    ...
    def simulate(self, n_paths: int = 10_000, seed: Optional[int] = 42) -> np.ndarray:
        ...
        paths = self.S0 * np.exp(np.hstack([np.zeros((n_paths, 1)), log_paths]))
...
class MonteCarloEngine:
    ...
    def run(
        self, S0_vec: Optional[np.ndarray] = None, seed: int = 42
    ) -> MonteCarloResult:
        """
        ...
        final_values = (
            (self.weights * final_prices).sum(axis=1)
            * V0
            / np.dot(self.weights, S0_vec)
        )
        pnl = final_values - V0
    ...
    def fit(self) -> Dict:
        ...
        for col in assets.columns:
            ...
            results[col] = {
                ...
                "treynor": round(
                    (assets[col].mean() * 252 - self.rf * 252) / (beta + 1e-12), 4
                ),
            }
...
class GARCH:
    ...
    def fit(self):
        ...
        self.model_ = arch_model(
            self.returns, vol="Garch", p=1, q=1, mean="Constant", dist="normal"
        )
        ...
        return {
            ...
            "long_run_vol": round(
                float(
                    np.sqrt(
                        p.get("omega", 1e-6)
                        / max(1e-12, 1 - p.get("alpha[1]", 0) - p.get("beta[1]", 0))
                    )
                    / 100
                    * np.sqrt(252)
                ),
...
class PCAFactorModel:
    ...
    def factor_returns(self, returns: pd.DataFrame) -> pd.DataFrame:
        ...
        F = X @ self.eigvecs.T
        return pd.DataFrame(
            F, index=returns.index, columns=[f"PC{i+1}" for i in range(self.k)]
        )
...
class BayesianVaR:
    ...
    def _posterior_params(self):
        ...
        beta_n = (
            beta_0 + 0.5 * S + 0.5 * kappa_0 * self.n * (x_bar - mu_0) ** 2 / kappa_n
        )
        return mu_n, kappa_n, alpha_n, beta_n

    def posterior_var(self, confidence: float = 0.95, n_samples: int = 20_000) -> Dict:
        """
        ...
        samples = stats.t.rvs(
            df_t, loc=mu_n, scale=scale, size=n_samples, random_state=0
        )
        losses = -samples
