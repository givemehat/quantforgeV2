...
from config import (
    NSE_API_BASE, NSE_TICKERS, US_TICKERS, CRYPTO_TICKERS,
    DB_PATH, CACHE_TTL, MARKET_OPEN, MARKET_CLOSE, POLL_INTERVAL,
    TRADING_DAYS
)
    ...
    def upsert_ohlcv(self, ticker: str, df: pd.DataFrame):
        ...
        for date, row in df.iterrows():
            self.conn.execute(
                ...
                (ticker, date, row.get("Open", np.nan), row.get("High", np.nan),
                 row.get("Low", np.nan), row.get("Close", np.nan), row.get("Volume", 0))
            )
        self.conn.commit()
    ...
    def fetch_nse_quote(ticker: str) -> Optional[Dict]:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                return {
                    "ticker":  ticker,
                    "price":   float(data.get("lastPrice", data.get("data", [{}])[0].get("lastPrice", 0))),
    ...
    def fetch_yf_quote(ticker: str) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                "pct":     float((info.last_price / info.previous_close - 1) * 100 if info.previous_close else 0),
    ...
    def get_historical(
        ...
        for ticker in tickers:
            if ticker in closes.columns:
                df_t = raw.xs(ticker, axis=1, level=1) if isinstance(raw.columns, pd.MultiIndex) else raw
